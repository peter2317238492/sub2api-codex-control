  ...
  <div v-if="open" class="modal-backdrop" @click.self="$emit('close')">
    <form class="dialog new-thread-dialog" @submit.prevent="submit">
      ...
      <label class="field-label">
        <span>工作目录</span>
        <span class="select-field"><FolderOpen :size="16" /><select v-model="cwd" required><option v-for="root in roots" :key="root" :value="root">{{ root }}</option></select></span>
      </label>
      <label class="field-label">
        <span>模型</span>
        <select v-model="model" class="standalone-select">
      ...
      <footer>
        ...
        <button class="primary-button" type="submit" :disabled="!cwd">创建</button>
      </footer>
    </form>
  </div>
</template>
