...
| --- | --- | --- |
| Control API | `0.1.1` (`control-v0.1.0` burned: its pipeline run failed because the release validate job never installed the pinned Codex CLI that `scripts/verify-contracts.mjs` requires — and formal tags are immutable) | API image digest and source commit recorded |
| PWA | `0.1.1` (versioned together with the Control API release tag) | PWA image digest built from the same release commit |
