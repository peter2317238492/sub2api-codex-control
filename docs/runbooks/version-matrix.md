...
| --- | --- | --- |
| Control API | `0.1.12` candidate: fixes Connector metadata/aggregate serialization, first-install backup, isolated PostgreSQL/Redis recovery, and Redis ACL admission; frozen against Sub2API `0.2.1`. Published `control-v0.1.11` remains the prior artifact release. | Candidate requires a new signed release and production acceptance |
